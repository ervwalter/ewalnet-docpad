// Inspired by SignalR (MIT/X11 License)
...
namespace Lokad.Cloud.Storage
{
    internal static class TaskAsyncHelper
    {
        ...
        public static TTask Catch<TTask>(this TTask task, IStorageObserver observer, Action<Exception> handler) where TTask : Task
        {
            if (task != null && task.Status != TaskStatus.RanToCompletion)
            {
                task.ContinueWith(innerTask =>
                {
                    var ex = innerTask.Exception;
                    observer.Notify(new TaskFailedEvent(ex));
                    handler(ex);
                }, TaskContinuationOptions.OnlyOnFaulted);
            }
            return task;
        ...
        {
            // TODO: Replace with Task.Delay on NETFX_CORE
            var tcs = new TaskCompletionSource<object>();
